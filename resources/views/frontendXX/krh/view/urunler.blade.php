...
@section('getTitle')  {{$kategori->baslik}}  @endsection

@section('getDescription')    {{$kategori->baslik}}  @endsection
        ...
        <div class="container">
            <div class="row align-items-center justify-content-center small-screen">
                <div class="col-lg-8 position-relative text-center page-title-extra-large" data-anime='{ "el": "childs", "translateY": [50, 0], "opacity": [0,1], "duration": 600, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                    <h1 class="m-auto text-white text-shadow-double-large fw-600 ls-minus-2px">    {{$kategori->baslik}} </h1>
                </div>
                    ...
                    <div class="col-12" data-anime='{ "translateX": [0, 0], "opacity": [0,1], "duration": 1200, "delay": 0, "staggervalue": 300, "easing": "easeOutQuad" }'>
                        <div class="swiper slider-one-slide magic-cursor dark" data-slider-options='{ "slidesPerView": 1, "spaceBetween": 25, "loop": true, "navigation": { "nextEl": ".slider-one-slide-next-1", "prevEl": ".slider-one-slide-prev-1" }, "autoplay": { "delay": 30000, "disableOnInteraction": false }, "keyboard": { "enabled": true, "onlyInViewport": true }, "breakpoints": { "1400": { "slidesPerView": 1 }, "1200": { "slidesPerView": 1 }, "992": { "slidesPerView": 1 }, "768": { "slidesPerView": 1 }, "320": { "slidesPerView": 1 } }, "effect": "slide" }'>
                            <div class="row ">
                                ...
                                <div class="col-6 team-style-08 border-radius-6px ">
                                    <figure class="mb-30 position-relative overflow-hidden border-radius-4px">
                                        <img src="{{ asset(isset($resimler[$urun->parentId][0]) ? $resimler[$urun->parentId][0] : '') }}" alt="" />
                                        <figcaption class="w-100 h-100 d-flex align-items-end p-13 lg-p-8 md-p-10 bg-gradient-base-transparent border-radius-6px">
                                            <div class="w-100">
                                                <span class="team-member-name fw-500 text-white d-block">{{ $urun->baslik }}</span>
                                            
                                            </div>
                                            <div class="social-icon d-flex flex-column flex-shrink-1">
                                                <a href="{{route('front.urundetay',['id' => $urun->url])}}" target="_blank" class="text-dark-gray bg-white">
